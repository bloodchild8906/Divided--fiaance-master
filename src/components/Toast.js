...
const Toast = ({ toast, onRemove }) => {
  ...
  return (
    <div className={getToastStyles()}>
      ...
      <button
        type="button"
        className="ml-auto -mx-1.5 -my-1.5 rounded-lg focus:ring-2 p-1.5 inline-flex h-8 w-8 hover:bg-white hover:bg-opacity-20 focus:outline-none"
        onClick={handleRemove}
        aria-label="Close"
      >
        <span className="sr-only">Close</span>
        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 14 14">
          <path fillRule="evenodd" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" clipRule="evenodd" />
        </svg>
      </button>
    </div>
  );
